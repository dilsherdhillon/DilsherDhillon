...
If you've read my previous blog, you've probably figured out my love for the brms[link] package and the tidyverse set of packages.  So let's load them in.  
...
The mgcv library has a really neat function `gamSim`, which let's you simulate non-linear data.  Let's simualate some data, and use the `eg` 4, which simluates data by group (3 groups by default). For more information on this check out the spline help page[link]
